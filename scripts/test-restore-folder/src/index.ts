...
const JOB_PROFILE = "RestoreFolder";
...
async function startJob(resourceManager: ResourceManager, folder: Locator, priority: RestorePriority) {
...
async function testJob(resourceManager: ResourceManager, folder: Locator, priority: RestorePriority) {
    let job;
    ...
    log("Creating job");
    job = await startJob(resourceManager, folder, priority);
...
async function main() {
    ...
    const folder = new S3Locator({ url: await buildS3Url(awsArchiveBucket, prefix, s3Client) });
    log(folder);

    await testJob(awsResourceManager, folder, RestorePriority.High);
}
