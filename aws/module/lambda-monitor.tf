...
locals {
  lambda_name_monitor = format("%.64s", replace("${var.prefix}-monitor", "/[^a-zA-Z0-9_]+/", "-"))
}

resource "aws_iam_role" "monitor" {
  name = format("%.64s", replace("${var.prefix}-${var.aws_region}-monitor", "/[^a-zA-Z0-9_]+/", "-"))
    ...
    Statement = concat([
      ...
      },
      {
        ...
        Resource = concat([
          ...
          "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${local.lambda_name_monitor}:*",
...
resource "aws_lambda_function" "monitor" {
  ...
  filename         = "${path.module}/lambdas/monitor.zip"
  function_name    = local.lambda_name_monitor
  role             = aws_iam_role.monitor.arn
  handler          = "index.handler"
  source_code_hash = filebase64sha256("${path.module}/lambdas/monitor.zip")
  runtime          = "nodejs22.x"
  timeout          = "900"
  memory_size      = "2048"
...
resource "aws_cloudwatch_event_rule" "monitor" {
  name                = format("%.64s", "${var.prefix}-monitor")
