    ...
    properties = {
      ...
      siteConfig = {
        appSettings = [
          ...
          {
            name  = "MCMA_SERVICE_REGISTRY_URL",
            value = var.service_registry.service_url
          },
          {
            name  = "MCMA_SERVICE_REGISTRY_AUTH_TYPE",
          ...
          },
          {
            ...
            value = var.job_profile_prefix
          },
          {
            name  = "STORAGE_CLIENT_CONFIG_SECRET_ID",
            value = azurerm_key_vault_secret.storage_client_config.name
          },
          {
            name  = "STORAGE_CLIENT_CONFIG_HASH",
            value = sha256(azurerm_key_vault_secret.storage_client_config.value)
          },
          {
          ...
          {
            name  = "MULTIPART_SIZE",
            value = tostring(var.multipart_size)
          },
          {
            name  = "AzureFunctionsJobHost__functionTimeout",
            value = var.worker_function_timeout != null ? var.worker_function_timeout : var.use_flex_consumption_plan ? "01:00:00" : "00:10:00"
          },
          {
